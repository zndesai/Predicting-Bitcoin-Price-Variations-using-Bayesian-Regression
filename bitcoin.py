...
def computeDelta(wt, X, Xi):
    ...
    for rowindex in xrange(0,len(Xi)):
       xi = Xi.iloc[rowindex][:-1]  #Excluding last column
       yi = Xi.iloc[rowindex][-1]   #Taking Yi
       ...
       a = X[:-1]
       b = xi
       #Similarity function of equation 9
       for value in range(0,len(b)):
         summation = summation + ((a[value] - meana) * (b[value] - meanb)) 
       
       similarity = summation/(len(b)*stda*stdb)
       summation = 0.0
       
       numerator = numerator + float(yi*math.exp(similarity*wt))
       denomenator = denomenator + float(math.exp(similarity*wt))
